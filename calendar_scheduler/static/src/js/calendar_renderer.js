odoo.define('web.CalendarRenderer', function (require) {
    ...
    // Get Calendar-n resource list
    var resource_values = [];
    var timeline_starttime = '';
    var timeline_endtime = '';
    var timeline_interval = '';
    var timeline_lunchstarttime = '';
    var timeline_lunchendtime = '';
    rpc.query({
        model: 'ebooking_auto.resource',
        method: 'resource_list',
    }).then(function (res) {
        resource_values = [];
        _.each(res[0], function (rec){
            var resource_name = rec.name;
            if(!resource_name) resource_name='';
            else resource_name=' '+resource_name;
            resource_values.push({
                'id': rec.num.toString(),
                'title': rec.name,
            });
        });
        timeline_starttime = res[1];
        timeline_endtime = res[2];
        timeline_interval = res[3];
        timeline_lunchstarttime = res[4];
        timeline_lunchendtime = res[5];
    });
    ...
    return AbstractRenderer.extend({
        ...
        _eventRender: function (event) {
            ...
            var temp_name = "calendar-box";
            if (_.isEmpty(qweb_context.record)) {
                return '';
            } else {
                if(event.view == "timeline") temp_name = "calendar-box-timeline";
                return (this.qweb || qweb).render(temp_name, qweb_context);
            }
        },
        /**
         * @param {any} record
        ...
        _initCalendar: function () {
            ...
            var fc_options = $.extend({}, this.state.fc_options, {
                ...
                //     agenda: 'HH:mm'
                // },
                minTime: timeline_starttime,
                maxTime: timeline_endtime,
                slotDuration: '00:'+timeline_interval+':00',
                resourceAreaWidth: '10%',
                resourceLabelText: 'Өргөгч',
                resources: JSON.parse(JSON.stringify(resource_values)),
                aspectRatio: 2.0,
                allDaySlot:false,
            });
        ...
        _render: function () {
            ...
            switch (this.state.scale) {
                ...
                case 'timeline': $a = this.$small_calendar.find('a.ui-state-active'); break;
            }
